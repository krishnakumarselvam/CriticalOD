function [xk_New, mk_New, fQ_TTSDNew, fQ_New, fQ_SDALL] = fmincon_totTimeGamma_someMuExog_cf6_Phi_quad30_i(xk,loadX0_inF,qgParamsF,nwkF,delta,betaFile,outFilePrint,saveX0_outF,fctnTol,constrTol,phases2Remove,tradeoff)
%This is what I need
...
load(qgParamsF,'myOptions','minGreenTimeSec','term_index','non_term_index','nbSucc','maxSucc','succAll',...
    'nbPred','maxPred','predAll','nb','nbVbles','mu_index','last_index','nbPhases','nbSignNodes','nbSignQs');

load(nwkF,'gamma_s','buffer_s','pij_s','mu_s','freeFlowCap','availableGreenTimes_s','fixedGreenTime','phase2Node',...
...
if scaleF > 1
    ...
    alpha = betaOpt(alphaIndx)/scaleF;
...
null_beq = find(beq(nbSignNodes+(1:nbSignQs))==0);%%those queues do not have fixed green
indx1 = find(abs(Aeq(nbSignNodes+null_beq,:)*(x_init'))>constrTol)';%%mu-sum (queue belongs to which phase (green)
nonNull_beq = find(beq(nbSignNodes+(1:nbSignQs))~=0);%%those have fixed green
indx2 = find(abs(Aeq(nbSignNodes+nonNull_beq,:)*(x_init')-beq(nbSignNodes+nonNull_beq))>constrTol)';
diffLines = [];
...
%allG_z2 = sum(gamma(hasGammaInd_z2).*(1-x_out(nb*3 + hasGammaInd_z2)));
allNi = rho.*(1./(1-rho)-( ((buffer+2).*(rho.^(buffer+1)))./(1-rho.^(buffer+2)) ));
%fQ_New = 3600.*(allNi_z2./(condFact.*allG_z2));
allG_zsd = x_out(nb*2+(1:nb)).*(1-x_out(nb*3 + (1:nb)));
...
EX2=(1./x_out(nb+(1:nb))*1/(condFact/3600))'.^2.*(1-rho')./(1-rho'.^(buffer'+1)).*sum(sumOut(:,:),2);
...
fQ_TTSDNew = fQ_New+tradeoff*fQ_SDALL;%new objective
...
%%%%%%%%%%%%%%%%%%%%%%%%
    function [z, z_grad] = objFctn(y0)
        ...
        allNi_zsd = rho.*(1./(1-rho)-(((buffer+2).*(rho.^(buffer+1)))./(1-rho.^(buffer+2))));
        
        allG_zsd = y0(nb*2+(1:nb)).*(1-y0(nb*3 + (1:nb)));

        %EX=3600*allNi_z2/(allG_z2*condFact);
        
        
        EXSD=allNi_zsd./(allG_zsd*(condFact/3600));%%EX FOR EACH QUEUE
        EX=sum(EXSD);
        E2X = EXSD.^2;
        sumOut=zeros(nb,b);
        rhotmp=repmat(rho',1,b);
        ...
        EX2=(y0(nb+(1:nb))*1/(condFact/3600))'.^2.*(1-rho')./(1-rho'.^(buffer'+1)).*sum(sumOut(:,:),2);
        ...
        gt_indx1 = gt_indx;
        gt_indx1(phases2Remove) = [];
        z_phi = aBeta + y0(gt_indx1)*bBeta+(y0(gt_indx1).^2)*cBeta;
        
        z = alpha*(EX+tradeoff*SDALL) + z_phi;
        
        %gradient

        buffertmp=repmat(buffer',1,b);
        sumderivmu=(((2+om).*rhotmp.^(1+om)+(buffertmp-om-1).*rhotmp.^(buffertmp+om+2)...
                          -(om+3).*rhotmp.^(om+2)+(om-buffertmp+2).*rhotmp.^(buffertmp+om+3))./(1.-rhotmp.^(buffertmp+1)).^2).*sumInside;
        sumderivlambda=(((om.*rhotmp.^(om-1)+(buffertmp-om+1).*rhotmp.^(buffertmp+om)...
                          -(om+1).*rhotmp.^om+(om-buffertmp).*rhotmp.^(buffertmp+om+1)))./(1.-rhotmp.^(buffertmp+1)).^2).*sumInside;

        derivmu=1./y0(2*nb+(1:nb)).*(sum(sumderivmu(:,:),2))'; %%VAR derivative w.r.t. mu
        derivlambda=y0(nb+(1:nb)).^3.*(sum(sumderivlambda(:,:),2))';%%VAR derivative w.r.t. lambda
        
        %gradient EX+SD
        derivElambda = (y0(nb+(1:nb))).^2./(1-y0(3*nb+(1:nb))).*(1./((1-rho).^2) - (((buffer+2).*(buffer+1)).*(rho.^buffer)+(buffer+2).*(rho.^(2*buffer+2)))./((1-(rho.^(buffer+2))).^2));%%EX derivative w.r.t.lambda
        derivRho = 1./((1-rho).^2) - (((buffer+2).^2).*(rho.^(buffer+1)))./((1-(rho.^(buffer+2))).^2);
        jac_val_Q = (alpha*3600/condFact)*[(allNi_zsd.*y0(nb*2+(1:nb)))./((allG_zsd).^2)-0.5*tradeoff*(VARALL.*(condFact/3600)^2)^(-0.5).*2.*allNi_zsd./(allG_zsd)...
            .*(allNi_zsd./(y0(nb*2 + (1:nb)).*(1-y0(3*nb+(1:nb))).^2)),...
            derivRho./(1-y0(3*nb+(1:nb)))+0.5*tradeoff*(VARALL.*(condFact/3600)^2)^(-0.5)...
            .*(derivmu-2.*(allNi_zsd./allG_zsd).*derivRho./(1-y0(3*nb+(1:nb)))),...
            derivElambda+0.5*tradeoff*(VARALL.*(condFact/3600)^2)^(-0.5)...
            .*(derivlambda-2.*(allNi_zsd./allG_zsd).*derivElambda)];

        jac_indx_Q = [nb*3+(1:nb),nb+(1:nb),nb*2+(1:nb)];

        
        jac_val_phi = bBeta + 2.*(y0(gt_indx1)').*cBeta;
        jac_indx_phi = gt_indx1;
        
        % ensure that indexes of gradients are not overlapping
        if ~isempty(intersect(jac_indx_Q,jac_indx_phi)) error('need to calc jac val as a combination of both jacobians'); end;
        z_grad = sparse(1,[jac_indx_Q,jac_indx_phi],[jac_val_Q,jac_val_phi'],1,nbVbles);
        % hessian
...
%%%%%%%%%%%%%%%%%%%%%%%%
    function [c, eq_out, grad_c, jacob] = nonLinConstr(s0)
        ...
        sumT = sqrt(sum((s0(gt_indx)-xk).^2));
        c = sumT - delta;
        grad_c = sparse(gt_indx,1,(s0(gt_indx)-xk)/sumT,nbVbles,1);
