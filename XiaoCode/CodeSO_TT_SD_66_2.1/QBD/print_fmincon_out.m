function print_fmincon_out(fval, exitflag, output, lambda, outFile)
...
fprintf(fid1, '%s   ','firstorderopt:');    fprintf(fid1,'%1.4g\n',output.firstorderopt); 
fprintf(fid1, '%s   ','message:');          fprintf(fid1,'%s\n\n\n',output.message); 
%{
[f_row,f_col] = size(lambda.eqnonlin);
fid1 = fopen(outFile,'a');
...
for i = 1:f_col str_tmp =  ['%1.6f    ',str_tmp]; end;
for i = 1:f_row
    fprintf(fid1,str_tmp,lambda.eqnonlin(i,:));
end;
fprintf(fid1,'\n\n');
clear str_tmp;
%}
